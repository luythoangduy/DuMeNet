...
def build_position_embedding(pos_embed_type, feature_size, hidden_dim):
    ...
    return pos_embed __init__(self, mem_dim, feature_dim, **kwargs):
...
class UniADMemory(nn.Module):
    def __init__(
        ...
        save_recon,
        initializer,
        upsample_config=None,  # New parameter for upsampling config
        **kwargs,
    ):
        super().__init__()
        ...
        # Upsampling - NEW: Configurable upsampling
...
# [Rest of the classes remain the same - TransformerEncoder, TransformerDecoder, etc.]
